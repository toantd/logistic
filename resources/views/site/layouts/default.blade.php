...
<head>
    <meta charset="utf-8"/>
    <title>@section('title') SHD connect @show</title>
    @section('meta_keywords')
        <meta name="keywords" content="your, awesome, keywords, here"/>
    @show @section('meta_author')
        <meta name="author" content="imtoantran"/>
    @show @section('meta_description')
        <meta name="description"
              content="Share to get more."/>
        @show
                    ...
                    @if (Auth::check())
                        @if(isset($admin))
                            <li><a href="{{{ URL::to('admin/dashboard') }}}">
                                    {{{Lang::get('site/site.admin_panel') }}}
                                </a>
                            </li>
                        @endif
                        <li>
                            <a href="#">{{{ Lang::get('site/site.login_as') }}}
                                {{{Auth::user()->name }}}
                            </a>
                        </li>
                        <li>
                            <a href="{{{ URL::to('auth/logout') }}}">
                                {{{Lang::get('site/site.logout') }}}</a>
                        </li>
                    @else
                        <li {{ (Request::is('auth/login') ? ' class="active"' : '') }}>
                            <a href="{{{ URL::to('auth/login') }}}">
                                {{{Lang::get('site/site.login') }}}
                            </a>
                        </li>
                        <li {{ (Request::is('auth/register') ? ' class="active"' : '') }}><a
                                    href="{{{ URL::to('auth/register') }}}">{{{
                                                        Lang::get('site/site.sign_up') }}}</a></li>
                    @endif
                </ul>
            </div>
        </div>
